...
const versions=(a.versions||[]).map((v,i)=>`<div class="version-row"><div><b>${esc(v)}</b><span>${i===0?'Latest release':'Previous release'}</span></div><a class="btn ${i===0?'primary':'ghost'}" href="${esc(i===0?(a.download||a.link||'#'):(a.link||'#'))}" target="_blank" rel="noopener">Download ↗</a></div>`).join('');
